...
public interface DefaultMapper {
	...
	@Results(id = "sysFileResult", value = {
	  ...
	  @Result(property = "linenbr", column = "line_nbr"),
	...
	SysFileInfoMessage getMessageByControllerSysKeyLineNbr(String controller, String sysKey, int line_nbr);

	@ResultMap("sysFileResult")
	@Select("select * from sysfileoutbox where id=#{id}")
	SysFileInfoMessage getMessageById(int id);
	...
	Integer checkExistMessageByControllerSyskeyLinenbr(String controller, String sysKey, int linenbr);
	
	@Select("select 1 from sysfileoutbox where id = #{id}}")
	Integer checkExistMessageById(int id);
	...
	@Update(value = { "update sysfileoutbox set update_date = '${updateDate}', status = '${status}', err_message = '${errMsg}'"
			+ " where id = ${id}"})
	boolean updateMessage(SysFileInfoMessage message);
